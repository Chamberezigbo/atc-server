...
export async function createBooking(req, res) {
  ...
  for (const item of lineItems) {
    ...
    if (!Number.isInteger(quantity) || quantity < 1) {
      return res.status(400).json({ error: `Invalid quantity for ${service.name}` })
    }
    if (service.pricingType === 'FLAT' && quantity !== 1) {
      return res.status(400).json({ error: `${service.name} can only be booked once` })
    }

    resolvedLineItems.push({
      serviceId: service.id,
      description: service.tierLabel ? `${service.name} — ${service.tierLabel}` : service.name,
      quantity,
